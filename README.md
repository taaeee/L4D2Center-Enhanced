### Features

- Launch anticheat and login automatically.
- Autoready button.
- Custom themes.
- Friend list and party invitations.
- Streamer mode.
- OBS overlay for mmr and match info.
- Add players to avoid list.


# L4D2Center Enhanced

![](https://64.media.tumblr.com/e336d963d32f44524c81127ef14de93d/5142b4296095f390-ce/s540x810/5f2d9b54071b7a48f735ff685a9d2ca2f8bdb895.pnj)

## Download
[⬇️ DOWNLOAD](https://github.com/taaeee/L4D2Center-Enhanced/releases/lastest)
## Table of Contents

- [How to install extension](#how-to-install-extension)
- [How to configure the launcher](#how-to-configure-the-launcher)
- [Video tutorial](#video-tutorial)

## How to install extension
**Clone repository or [Download Lastest Version](https://github.com/taaeee/L4D2Center-Enhanced/releases/lastest)**

1. Extract the **Extension.zip**.
2. Open your browser in **extensions tab**.
3. Enable **Developer mode**.
4. Click on **Load unpacked** and select the **dist** extracted folder.
5. Reload l4d2center.com.

## How to configure the launcher
1. Open the **native_host** folder.
2. Launch **install_host.bat**
3. Find your extension ID in your extension popup or extensions tab.
4. Paste it in the program and press Enter.
5. Go to your extension popup(the icon in taskbar)
6. Click on the folder button to find your anticheat.exe
7. Click the save button to save the path to the .exe file.
8. Reload the extension and the l4d2center page.

## Video tutorial
[Video Tutorial](https://www.youtube.com/watch?v=oEvuccT6Pqk)

